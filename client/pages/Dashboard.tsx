      ...
      <div className="container py-4 px-4 sm:py-8">
        {userProfile && (
          <div className="mb-6 sm:mb-8 glass-card p-4 sm:p-6">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-gradient-to-br from-praxis-purple to-praxis-blue" />
              <div>
                <h1 className="font-heading text-lg sm:text-xl">Welcome back, {userProfile.name}!</h1>
                <p className="text-white/70 text-xs sm:text-sm">{userProfile.email}</p>
              </div>
            </div>
                ...
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {userProfile.skills.length > 0 ? (
                    userProfile.skills.map((skill, index) => (
                      <span key={`my-skill-${index}`} className="px-2 py-1 rounded-md bg-praxis-green/20 border border-praxis-green/40 text-xs text-praxis-green">{skill}</span>
                    ))
                  ) : (
                ...
                <div className="flex flex-wrap gap-1.5 sm:gap-2">
                  {userProfile.wantsToLearn.length > 0 ? (
                    userProfile.wantsToLearn.map((skill, index) => (
                      <span key={`my-learn-${index}`} className="px-2 py-1 rounded-md bg-praxis-blue/20 border border-praxis-blue/40 text-xs text-praxis-blue">{skill}</span>
                    ))
                  ) : (
        ...
        {connectionRequests.length > 0 && (
          <div className="mb-6 sm:mb-8">
            <h2 className="font-heading text-xl sm:text-2xl mb-3 sm:mb-4">Connection Requests</h2>
            <div className="grid gap-3 sm:gap-4">
              {connectionRequests.map((request) => (
                <div key={request.id} className="glass-card p-3 sm:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div>
                      <div className="font-heading text-base sm:text-lg">Someone wants to connect!</div>
                      <div className="text-xs text-white/60">Tap accept to start chatting</div>
                    </div>
                  </div>
                  <div className="flex gap-2 sm:gap-3 w-full sm:w-auto">
                    <button 
                      className="btn-secondary px-3 py-2 text-sm flex-1 sm:flex-none" 
                      onClick={() => declineRequest(request.id)}
                    >
                      Decline
                    </button>
                    <button 
                      className="btn-primary px-3 py-2 text-sm flex-1 sm:flex-none" 
        ...
        <h2 className="font-heading text-xl sm:text-2xl">Discover</h2>
        ...
        {loading ? (
          <div className="mt-6 text-center">
            <div className="text-white/50">Loading matches...</div>
          </div>
        ) : (
            ...
            <AnimatePresence initial={false}>
              {matchedUsers.filter((u) => {
                ...
                <motion.div
                  ...
                  whileHover={{ y: -2 }}
                  className="glass-card p-4 sm:p-5 flex flex-col transition-transform"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="font-heading text-base sm:text-lg truncate">{u.name}</div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-1.5">
                    {u.skills.filter(skill => userProfile?.wantsToLearn.includes(skill)).slice(0, 3).map((skill) => (
                      <span key={`${u.id}-teach-${skill}`} className="px-2 py-1 rounded-md bg-praxis-green/20 border border-praxis-green/40 text-xs text-praxis-green">{skill}</span>
                    ))}
                  ...
                  <div className="mt-1 flex flex-wrap gap-1.5">
                    {u.wantsToLearn.filter(skill => userProfile?.skills.includes(skill)).slice(0, 3).map((skill) => (
                      <span key={`${u.id}-learn-${skill}`} className="px-2 py-1 rounded-md bg-praxis-blue/20 border border-praxis-blue/40 text-xs text-praxis-blue">{skill}</span>
                    ))}
                  ...
                  <div className="mt-4 sm:mt-5 flex gap-2 sm:gap-3">
                    <button className="btn-secondary w-full text-sm py-2" onClick={() => onPass(u.id)}>Pass</button>
                    <button className="btn-primary w-full text-sm py-2" onClick={() => onLike(u.id)}>Say hi</button>
                  </div>
                </motion.div>
              ))}
            </AnimatePresence>
